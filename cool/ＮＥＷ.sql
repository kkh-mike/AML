...
BEGIN
    ...
    */
    ...
    ;WITH TargetTable AS
    (
        SELECT * FROM BSADB.dbo.OFAC_GLSpec WHERE GROUPID = @GroupId
    )
    ...
    */
    ...
    TempNUM AS
    (
        SELECT spec_name, updated_date, remark, ent_num
    ...
    SELECT 
        E.ent_num, A.EntNum, A.SdnName, A.UpdatedDate,
        B.Address, LEFT(B.City,100), LEFT(B.AddressCountry,100),
        C.IdentificationType, LEFT(C.IdentificationNumber,44),
        CASE WHEN D.InfoDay IS NULL OR D.InfoMonth IS NULL 
             THEN D.InfoYear 
    ...
    /*
        ============================================================
        第三段：Sanctions IDs 同步（GROUPID = 10）← 已優化 + Debug
        目標表：BSADB.dbo.OFAC_GLSpec（適合海外 GL 版）
        設計說明：
          - 只同步指定的 IdentificationType
          - 用 remark (EntityID) + spec_name (ID Number) + IdentificationType 做唯一鍵
          - ADDRESS 存放 IdentificationType + 固定後綴
          - 新資料自動產生 ent_num
        ============================================================
    */
    ...
    -- 產生新的 ent_num（從目前最大值往後編）
    SELECT @MAXNo = ISNULL(MAX(ent_num), 0) FROM BSADB.dbo.OFAC_GLSpec;
    ...
    /* ============================================================
       Debug 區段（需要時把下面的註解拿掉即可）
       ============================================================
    SELECT '===== #tempDJBlacklistSID =====' AS DebugStep;
    SELECT * FROM #tempDJBlacklistSID ORDER BY EntNum, IdentificationType;

    SELECT '===== TempNUM (現有 GROUPID=10) =====' AS DebugStep;
    SELECT * FROM (
        SELECT spec_name, updated_date, remark, ent_num,
               REPLACE(ADDRESS, '; Please go to Dowjones website for more detail', '') AS IdentificationType
        FROM BSADB.dbo.OFAC_GLSpec 
        WHERE GROUPID = 10 AND DInd = 'N'
    ) T ORDER BY remark, IdentificationType;

    SELECT '===== #tempDJBlacklistDetailsSID (合併後) =====' AS DebugStep;
    SELECT * FROM #tempDJBlacklistDetailsSID ORDER BY EntNum, IdentificationType;
    ============================================================ */
