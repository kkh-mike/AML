...
/****** Object:  StoredProcedure [dbo].[GLSPEC_SyncDJToInternalList]    Script Date: 2026/7/17 下午 05:34:28 ******/
SET ANSI_NULLS ON
GO
...
功能說明：
    將 DJ (Dow Jones) 外部制裁名單資料同步至內部核心黑名單主表 (OFAC_GLSpec)。
    並透過 MERGE 的 OUTPUT 機制，自動記錄新增 (01)、修改 (02)、軟刪除 (03) 的歷程至 Log 表。
輸入參數：
    @GroupId BIGINT : 名單群組 ID，用以區分/隔離特定群組的黑名單。
========================================================================================
*/
CREATE PROCEDURE [dbo].[GLSPEC_SyncDJToInternalList]
    @GroupId BIGINT
AS
BEGIN
    -- 關閉受影響資料列計數訊息，提升執行效能
    ...
    */
    ...
    (
        EntNum      INT NOT NULL,           -- DJ 實體編號 (Entity Number)
        SdnName     NVARCHAR(350) NOT NULL, -- 制裁主體名稱 (Primary Name)
        UpdatedDate VARCHAR(8) NOT NULL     -- 資料更新日期 (YYYYMMDD)
    );

    -- 建立暫存表：存放串接各明細 (地址、證件、生日) 後的完整資訊
    CREATE TABLE #tempDJBlacklistDetails
    (
        EntNum       INT NOT NULL,          -- DJ 實體編號
        SdnName      NVARCHAR(350) NOT NULL,-- 制裁主體名稱
        UpdatedDate  VARCHAR(8) NOT NULL,   -- 更新日期
        Address      NVARCHAR(400),         -- 地址
        City         NVARCHAR(200),         -- 城市
        Country      NVARCHAR(100),         -- 國家
        IdentType    VARCHAR(2),            -- 證件類別代碼
        IdentNumber  NVARCHAR(44),          -- 證件號碼
        DOB          VARCHAR(8)             -- 出生年月日 (YYYYMMDD 或 YYYY)
    );

    -- =================================================================================
    -- 步驟 1：篩選 DJ 主要名稱資料，寫入 #tempDJBlacklist
    -- 說明：NamePropertyBitCode & 1 > 0 表示該筆名稱為 Primary Name (主要名稱)
    -- =================================================================================
    ...
    -- =================================================================================
    -- 步驟 2：利用 CTE 取得各實體的「第一筆」明細資料 (地址/證件/生日)，彙整寫入暫存明細表
    -- =================================================================================
    ;WITH TempAddress AS
    (
        -- 取得各實體最新/首筆有效地址 (依 Id 排序取 RowId = 1)
        SELECT 
            A.EntityId, 
            A.Address, 
            A.City, 
    ...
    INSERT INTO #tempDJBlacklistDetails
        (EntNum, SdnName, UpdatedDate, Address, City, Country, IdentType, IdentNumber, DOB)
    SELECT 
        A.EntNum,
        A.SdnName,
        A.UpdatedDate,
        ...
        -- 若缺少月或日，則僅保留年份；完整則格式化為 YYYYMMDD (樣式 112)
        CASE 
    ...
    -- =================================================================================
    -- 步驟 3：使用 MERGE 比對並同步至 OFAC_GLSpec 主表，同時輸出異動至 Log 表
    -- =================================================================================
    ;WITH TargetTable AS
    (
        -- 限制比對目標僅限指定 @GroupId 的現有名單
        SELECT 
            A.spec_name, A.updated_date, A.remark, A.TaxID, A.IDENT, A.IDENTO, A.TaxIDCD, A.IDNUM, A.DInd,
            A.creUserID, A.STATE, A.ADDRESS, A.COUNTRY, A.GROUPID, A.SOURCE, A.EnterFlag,
            A.ApprovedBy, A.ApprovedDate, A.DelUserID, A.DelDate, A.ApproveDelUserID, A.ApproveDelDate,
            A.creDate, A.updated_date_UTC, A.ApprovedDate_UTC, A.DelDate_UTC, A.ApproveDelDate_UTC,
            A.creDate_UTC, A.ent_num, A.DOB
   ...
   AND T.remark = CAST(S.EntNum AS VARCHAR(100))
    ...
    WHEN NOT MATCHED BY TARGET THEN
        ...
        VALUES (S.SdnName, S.UpdatedDate, S.EntNum, 'N', 'System Admin', ISNULL(S.City, ''), S.Address, S.Country,
                @GroupId, S.IdentType, S.IdentNumber, S.DOB, NULL, 'D', 'System Admin', GETDATE(),
                'System Admin', GETDATE(), 'System Admin', GETDATE(), GETDATE(),
                GETDATE(), GETUTCDATE(), NULL, NULL, GETUTCDATE())

    -- 情境 2：來源已不存在、目標仍為有效 -> 軟刪除 (標記 DInd = 'Y' 並填入刪除審核資訊)
    ...
    -- 情境 3：兩邊皆存在，但已被軟刪除 (DInd='Y') 或 資料日期有變更 -> 重新啟用/更新明細
    ...
    -- =================================================================================
    -- 步驟 4：輸出 MERGE 異動紀錄至 OFAC_GLSpec_Log
    -- ActionCD 代碼定義：
    --   '01' : 新增 或 重新啟用 (由 DInd='Y' 轉為 'N')
    --   '03' : 軟刪除 (DInd 轉為 'Y')
    --   '02' : 一般資料更新 (Update)
    -- =================================================================================
    OUTPUT 
    ...
    /*
        ...
        tempDJBlacklistSCO
        第2段：
                制裁控制與實質所有權名單（Sanctions Control & Ownership, SCO）
                目標表：BSADB.dbo.OFAC_GLSpec（固定 GROUPID = 27）
                稽核表：BSADB.dbo.OFAC_GLSpec_Log
                備註：拆分 Dow Jones SCO 名單
        ============================================================
    */
    ...
    -- 2-4. MERGE 同步 SCO 名單至 BSADB.dbo.OFAC_GLSpec (GROUPID = 27)
    ;WITH TargetTable AS (
        SELECT 
   ...
   AND T.remark = CAST(S.EntNum AS VARCHAR(100))
    ...
    /*
        ============================================================
        第3段：Sanctions IDs 同步（GROUPID = 10）
        來源：Dow Jones Sanctions IDs → BSADB.dbo.OFAC_GLSpec
        ============================================================
    */
    ...
    INTO BSADB.dbo.OFAC_GLSpec_Log
        (ent_num, spec_name, updated_date, remark, DInd, creUserID, IDENTO, STATE, ADDRESS, COUNTRY, GROUPID,
         ...
         CreationDate_UTC, ActionCD, ActionDt);
           ...
           /*
        ============================================================
        第4段：日本金融制裁名單同步（Japanese Finance）
